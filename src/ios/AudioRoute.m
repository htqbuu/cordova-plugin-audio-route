...
- (void) currentBluetoothDevices:(CDVInvokedUrlCommand*)command {
    CDVPluginResult* pluginResult;
    NSMutableArray* outputs = [NSMutableArray array];
    ...
    for (AVAudioSessionPortDescription* desc in [route outputs]) {
        ...
        if ([portType isEqualToString:AVAudioSessionPortBluetoothHFP]
            ...
            NSString* uid = [[desc UID] substringToIndex: 17];
            NSDictionary* dic = @{ @"name": [desc portName],
                                   @"id" : uid,
                                   @"type":portType
                                   };
            NSLog([NSString stringWithFormat:@"[currentBluetoothUIDs] uid = %@, name = %@", uid, [desc portName]]);
         
            [outputs addObject: dic];
        }
